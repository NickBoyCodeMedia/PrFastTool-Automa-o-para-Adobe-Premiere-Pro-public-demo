...
CSInterface.prototype.evalScript = function(script, callback)
{
    ...
    // Adicionar log de depuração mais detalhado
    var scriptPreview = script.length > 100 ? script.substring(0, 100) + '...' : script;
    console.log('[CSInterface] Executando script: ' + scriptPreview);
    console.log('[CSInterface] Timestamp: ' + new Date().toLocaleTimeString());
    
    try {
        window.__adobe_cep__.evalScript(script, function(result) {
            // Verificar resultado e adicionar diagnóstico
            if (result === 'EvalScript error.' || result === null || result === undefined) {
                console.error('[CSInterface] ERRO na execução do script: ' + scriptPreview);
                console.error('[CSInterface] O ExtendScript pode estar bloqueado ou houve um erro de sintaxe.');
                
                // Verificar se temos window.__adobe_cep__.getLastError
                if (window.__adobe_cep__.getLastError) {
                    var error = window.__adobe_cep__.getLastError();
                    console.error('[CSInterface] Detalhes do erro: ' + error);
                }
            } else {
                console.log('[CSInterface] Resultado recebido: ' + (typeof result === 'string' ? result.substring(0, 100) + (result.length > 100 ? '...' : '') : result));
            }
            
            callback(result);
        });
    } catch(e) {
        console.error('[CSInterface] Exceção ao executar evalScript: ', e);
